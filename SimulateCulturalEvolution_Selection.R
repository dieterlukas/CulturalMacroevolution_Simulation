#------------------------------------------------------------------------------------------
# Simulations for the manuscript - selection on a single variant
#------------------------------------------------------------------------------------------
...
#Add branch lengths to the tree based on Grafen's method (number of nodes between root and tip)
...
#For the simulations assuming that selection operated differently in different clades, 
...
#Create matrices for the reconstruction that assume that transitions to 'a' occurred more frequently
...
#Run the simulation generating tip data under a selection model, testing whether the reconstruction wrongly favours that the data fit a model of selection
...
# Phylogeny = Grafentree, Two Variants of the trait, Direction of Change = selection, Rate of Change = Slow

#need to still do 5,6,7,8,9 selectionvariants with onetree and with earlytree and with latetree
...
for (tree_variant in 1:length(tree_variants)) {
  ...
  for (selection_variant in 1:length(selection_variants)) {
    ...
    for (repetition in 1:repetitions) {
      ...
      #Simulate the data with the specified tree and the specified selection model
      ...
      if (length(table(simulatedtips_neutral))!=1){
        ...
        ifelse(selection_variant<4,selection_results[counter,2]<-2,selection_results[counter,2]<-4)
        ifelse(selection_variant<4,selection_results[counter,3]<-selection_variant,ifelse(selection_variant<7,selection_results[counter,3]<-selection_variant-3,selection_results[counter,3]<-selection_variant-6))
        ...
        # the phylogenetic reconstruction with the selection model, the model under which the data were simulated
        ...
        selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=simulatedtips_neutral, phy=currenttree, model=transitionmatrix)            
        
        selection_results[counter,10]<-selection_to_a_reconstruction_pathway_toone$opt$lnL
        ...
        selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=simulatedtips_neutral, phy=currenttree, model=transitionmatrix)            
        selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
        ...
        unconstrained_reconstruction <- fitDiscrete(dat=simulatedtips_neutral, phy=currenttree, model="ARD")            
        
        selection_results[counter,11]<-unconstrained_reconstruction$opt$lnL
        ...
        selection_results[counter,1]<-tree_variant
        ifelse(selection_variant<4,selection_results[counter,2]<-2,selection_results[counter,2]<-4)
        ifelse(selection_variant<4,selection_results[counter,3]<-selection_variant,ifelse(selection_variant<7,selection_results[counter,3]<-selection_variant-3,selection_results[counter,3]<-selection_variant-6))
        selection_results[counter,4]<-repetition
        selection_results[counter,22]<-selectionvariant_used
        ...
        if (length(table(cladeAsimulatedtips))!=1) { 
          ...
          selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=cladeAsimulatedtips, phy=cladeAtree, model=transitionmatrix)            
          ...
          selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=cladeAsimulatedtips, phy=cladeAtree, model=transitionmatrix)            
          selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
          
          
          
          # the phylogenetic reconstruction with an unconstrained model
          unconstrained_reconstruction <- fitDiscrete(dat=cladeAsimulatedtips, phy=cladeAtree, model="ARD")            
        ...
        if (length(table(cladeBsimulatedtips))!=1) { 
          ...
          selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=cladeBsimulatedtips, phy=cladeBtree, model=transitionmatrix)            
          ...
          selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=cladeBsimulatedtips, phy=cladeBtree, model=transitionmatrix)            
          
          
          # the phylogenetic reconstruction with an unconstrained model
          unconstrained_reconstruction <- fitDiscrete(dat=cladeBsimulatedtips, phy=cladeBtree, model="ARD")            
        ...
        if (length(table(cladeCsimulatedtips))!=1) { 
          ...
          selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=cladeCsimulatedtips, phy=cladeCtree, model=transitionmatrix)            
          ...
          selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=cladeCsimulatedtips, phy=cladeCtree, model=transitionmatrix)            
          selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
          ...
          # the phylogenetic reconstruction with an unconstrained model
          unconstrained_reconstruction <- fitDiscrete(dat=cladeCsimulatedtips, phy=cladeCtree, model="ARD")            
        ...
        if (length(table(cladeDsimulatedtips))!=1) { 
          ...
          selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=cladeDsimulatedtips, phy=cladeDtree, model=transitionmatrix)            
          ...
          selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=cladeDsimulatedtips, phy=cladeDtree, model=transitionmatrix)            
          selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
          ...
          # the phylogenetic reconstruction with an unconstrained model
          unconstrained_reconstruction <- fitDiscrete(dat=cladeDsimulatedtips, phy=cladeDtree, model="ARD")
        ...
        if (length(table(cladeEsimulatedtips))!=1) { 
          ...
          selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=cladeEsimulatedtips, phy=cladeEtree, model=transitionmatrix)            
          ...
          selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=cladeEsimulatedtips, phy=cladeEtree, model=transitionmatrix)            
          selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
          ...
          # the phylogenetic reconstruction with an unconstrained model
          unconstrained_reconstruction <- fitDiscrete(dat=cladeEsimulatedtips, phy=cladeEtree, model="ARD")
        ...
        if (length(table(cladeFsimulatedtips))!=1) { 
          ...
          selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=cladeFsimulatedtips, phy=cladeFtree, model=transitionmatrix)            
          ...
          selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=cladeFsimulatedtips, phy=cladeFtree, model=transitionmatrix)            
          selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
          ...
          # the phylogenetic reconstruction with an unconstrained model
          unconstrained_reconstruction <- fitDiscrete(dat=cladeFsimulatedtips, phy=cladeFtree, model="ARD")
        ...
        for (randomsample in 1:1) {
          ...
          if (length(table(ThreeQuartersimulatedtips))!=1) { 
            ...
            if(length(unique(ThreeQuartersimulatedtips))>1) ifelse(names(table(ThreeQuartersimulatedtips))[2]!=2, selection_results[counter,21]<-"TRUE",selection_results[counter,21]<-"FALSE")
            ...
            selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=ThreeQuartersimulatedtips, phy=ThreeQuartertree, model=transitionmatrix)            
            ...
            selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=ThreeQuartersimulatedtips, phy=ThreeQuartertree, model=transitionmatrix)            
            selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
            ...
            # the phylogenetic reconstruction with an unconstrained model
            unconstrained_reconstruction <- fitDiscrete(dat=ThreeQuartersimulatedtips, phy=ThreeQuartertree, model="ARD")            
        ...
        for (randomsample in 1:1) {
          ...
          if (length(table(Halfsimulatedtips))!=1) { 
            ...
            selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=Halfsimulatedtips, phy=Halftree, model=transitionmatrix)            
            ...
            selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=Halfsimulatedtips, phy=Halftree, model=transitionmatrix)            
            selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
            ...
            # the phylogenetic reconstruction with an unconstrained model
            unconstrained_reconstruction <- fitDiscrete(dat=Halfsimulatedtips, phy=Halftree, model="ARD")            
        ...
        for (randomsample in 1:1) {
          ...
          if (length(table(Rarelostsimulatedtips))!=1) { 
            ...
            selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=Rarelostsimulatedtips, phy=Rarelosttree, model=transitionmatrix)            
            ...
            selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=Rarelostsimulatedtips, phy=Rarelosttree, model=transitionmatrix)            
            selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
            ...
            # the phylogenetic reconstruction with an unconstrained model
            unconstrained_reconstruction <- fitDiscrete(dat=Rarelostsimulatedtips, phy=Rarelosttree, model="ARD")            
        ...
        for (randomsample in 1:1) {
          ...
          if (length(table(Frequentlostsimulatedtips))!=1) { 
            ...
            selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=Frequentlostsimulatedtips, phy=Frequentlosttree, model=transitionmatrix)            
            ...
            selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=Frequentlostsimulatedtips, phy=Frequentlosttree, model=transitionmatrix)            
            selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
            
            
            
            # the phylogenetic reconstruction with an unconstrained model
            unconstrained_reconstruction <- fitDiscrete(dat=Frequentlostsimulatedtips, phy=Frequentlosttree, model="ARD")            
        ...
        for (randomsample in 1:1) {
          ...
          selection_results[counter,5]<-"Forestloss"
          selection_results[counter,6]<-length(unique(Forestlosstips))
          ...
          if (length(table(Forestlosstips))!=1) { 
            ...
            if(length(unique(Forestlosstips))>1) ifelse(names(table(Forestlosstips))[2]!=2, selection_results[counter,21]<-"TRUE",selection_results[counter,21]<-"FALSE") 
            ...
            selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=Forestlosstips, phy=Forestlosttree, model=transitionmatrix)            
            ...
            selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=Forestlosstips, phy=Forestlosttree, model=transitionmatrix)            
            selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
            
            
            
            # the phylogenetic reconstruction with an unconstrained model
            unconstrained_reconstruction <- fitDiscrete(dat=Forestlosstips, phy=Forestlosttree, model="ARD")            
        ...
        for (randomsample in 1:1) {
          ...
          if (length(table(horizontal10tips))!=1) { 
            ...
            selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=horizontal10tips, phy=currenttree, model=transitionmatrix)            
            ...
            selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=horizontal10tips, phy=currenttree, model=transitionmatrix)            
            selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
            
            
            
            # the phylogenetic reconstruction with an unconstrained model
            unconstrained_reconstruction <- fitDiscrete(dat=horizontal10tips, phy=currenttree, model="ARD")            
        ...
        for (randomsample in 1:1) {
          ...
          if (length(table(cladeAsimulatedtips))!=1) {
            ...
            if (length(table(horizontalClade10tips))!=1) { 
              ...
              selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=horizontalClade10tips, phy=currenttree, model=transitionmatrix)            
              ...
              selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=horizontalClade10tips, phy=currenttree, model=transitionmatrix)            
              selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
              ...
              # the phylogenetic reconstruction with an unconstrained model
              unconstrained_reconstruction <- fitDiscrete(dat=horizontalClade10tips, phy=currenttree, model="ARD")            
        ...
        for (randomsample in 1:1) {
          ...
          selection_results[counter,5]<-"horizontalForest10"
          selection_results[counter,6]<-length(unique(horizontalForest10tips))
          ...
          if (length(table(horizontalForest10tips))!=1) { 
            ...
            if(length(unique(horizontalForest10tips))>1) ifelse(names(table(horizontalForest10tips))[2]!=2, selection_results[counter,21]<-"TRUE",selection_results[counter,21]<-"FALSE") 
            ...
            selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=horizontalForest10tips, phy=currenttree, model=transitionmatrix)            
            ...
            selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=horizontalForest10tips, phy=currenttree, model=transitionmatrix)            
            selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
            
            
            
            # the phylogenetic reconstruction with an unconstrained model
            unconstrained_reconstruction <- fitDiscrete(dat=horizontalForest10tips, phy=currenttree, model="ARD")            
        ...
        for (randomsample in 1:1) {
          ...
          if (length(table(horizontalNeighbor10tips))!=1) { 
            ...
            selection_to_a_reconstruction_pathway_toone <- fitDiscrete(dat=horizontalNeighbor10tips, phy=currenttree, model=transitionmatrix)            
            ...
            selection_to_a_reconstruction_pathway_fromone <- fitDiscrete(dat=horizontalNeighbor10tips, phy=currenttree, model=transitionmatrix)            
            selection_results[counter,18]<-selection_to_a_reconstruction_pathway_fromone$opt$lnL
            ...
            # the phylogenetic reconstruction with an unconstrained model
            unconstrained_reconstruction <- fitDiscrete(dat=horizontalNeighbor10tips, phy=currenttree, model="ARD")            
