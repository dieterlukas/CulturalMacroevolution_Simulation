...
drift_results<-matrix(data=NA,nrow=500000,ncol=6)
colnames(drift_results)<-c("Tree","Repetition","Sample","pvalue_ecology","pvalue_clade","variance_trait")
...
for (tree_variant in 1:length(tree_variants)) {
  ...
  if(tree_used=="Latetree") currenttree<-Latetree
    ...
    for (repetition in 1:repetitions) {
      ...
      simulatedtips_neutral<-fastBM(tree=currenttree,a=0,sig2=1)
...
#--#--#--#--#--#--#        #-#-#-#-#-#-#-#-#-#-#-#-#
  ...
  if(Option=="WNAI") write.csv(drift_results[1:counter,],file="SimulateCulture_DriftResults_Continuous_WNAI.csv")  
  if(Option=="PamaNyungan") write.csv(drift_results[1:counter,],file="SimulateCulture_DriftResults_Continuous_PamaNyungan.csv")  
